...
import { actionGetAuthBoardList } from '../../../../modules/action/BoardAction';
...
const InquiryUser = (props) => {
    ...
    const funcGetInquireList = () => {
        ...
        let temp = { where_key: 'BBS_KIND_CD', where_value: "M_QNA", where_type: 'equal' }
        ...
        if (answerYn && answerYn !== '') {
            let answerType = { where_key: 'ANSWER_YN', where_value: answerYn, where_type: 'equal' }
            whereOptions.push(answerType)
        }
        ...
        params.bbsKindCd = "M_QNA";
        params.whereOptions = whereOptions;
        params.pageLength = pageLimit;


        actionGetAuthBoardList(params).then((res) => {
    ...
    // [3] 페이지 이동
    const onDetail = (item) => {
        utilSetListSearch(inquiryuser)
        props.history.push(`/mypage/inquiry/user/detail/${item.id}`)
    }
                ...
                <div className="con-body">
                    ...
                    <div className="mydetail-info">
                        <ul className="infor-report gray100 mb30">
                            <li className="dot-txt">'고객센터 &#62; 이용문의'에서 등록하신 문의에 대한 답변을 확인할 수 있습니다.</li>
                            <li className="dot-txt">부적절한 게시물 또는 욕설, 비방 등의 게시물을 등록할 경우 게시물 삭제 또는 DTVERSE 이용이 제한 될 수 있습니다.</li>
                            <li className="dot-txt">임상 모집공고와 관련된 문의는 해당 모집공고 상세 내 1:1문의를 통해 문의 주시기 바랍니다.</li>
                        </ul>
                                            ...
                                            inquire?.map((item, index) => {
                                                ...
                                                return (
                                                    <tr key={index} onClick={() => onDetail(item)}>
                                                        ...
                                                        <td className="txt-left cursor" title={item.title} >{item.title}</td>
